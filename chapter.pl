...
use strict;
use warnings;
qw(c:/bin/perl c:/env/bin/perl /home/mckoskey/bin/perl);
...
foreach my $filename (@infiles)
{
	...
	print $file "\\chapter[" . $util->get_title($filename) . "]{" . $util->get_title($filename) . "}\\label{sec:" . $util->get_label ($filename) . "}\n";
	print $file "\n";
	print $file "\n";
